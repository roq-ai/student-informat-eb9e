...
import { getFacultyById } from 'apiSdk/faculties';
import { FacultyInterface } from 'interfaces/faculty';
import { CourseListPage } from 'pages/courses';

function FacultyViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<FacultyInterface>(
    () => (id ? `/faculties/${id}` : null),
    () =>
      getFacultyById(id, {
        relations: ['user'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('faculty', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/faculties/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Designation" text={data?.designation} />
