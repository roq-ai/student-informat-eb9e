...
import { getFaculties, deleteFacultyById } from 'apiSdk/faculties';
import { FacultyInterface } from 'interfaces/faculty';

type ColumnType = ColumnDef<FacultyInterface, unknown>;
...
export function FacultyListPage(props: FacultyListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFaculties({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'department.contains', 'designation.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FacultyInterface>>(
    () => `/faculties?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FacultyInterface) => {
    if (hasAccess('faculty', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/faculties/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/faculties/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('faculty', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/faculties/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('faculty', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="faculty" />
        </Flex>

        {hasAccess('faculty', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/faculties/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
