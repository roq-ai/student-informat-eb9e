...
export function CourseListPage(props: CourseListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCourses({
        relations: ['term', 'faculty', 'student.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('term', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'term',
          header: 'Term',
          accessorKey: 'term',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/terms/view/${record.term?.id}`}>
              {record.term?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('faculty', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'faculty',
          header: 'Faculty',
          accessorKey: 'faculty',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/faculties/view/${record.faculty?.id}`}>
              {record.faculty?.name}
            </Link>
          ),
        }
