...
import { getTerms, deleteTermById } from 'apiSdk/terms';
import { TermInterface } from 'interfaces/term';

type ColumnType = ColumnDef<TermInterface, unknown>;

interface TermListPageProps {
...
export function TermListPage(props: TermListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTerms({
        relations: ['university', 'course.count', 'student.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TermInterface>>(
    () => `/terms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TermInterface) => {
    if (hasAccess('term', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/terms/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'is_active', header: 'Is Active', accessorKey: 'is_active' },
    hasAccess('university', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'university',
          header: 'University',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/universities/view/${record.university?.id}`}
            >
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/terms/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('term', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/terms/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('term', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="term" />
        </Flex>

        {hasAccess('term', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
