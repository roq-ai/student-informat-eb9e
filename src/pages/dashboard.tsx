...
import { getFaculties } from 'apiSdk/faculties';
import { FacultyInterface } from 'interfaces/faculty';
import { getCourses } from 'apiSdk/courses';
import { CourseInterface } from 'interfaces/course';

import { FacultyListPage as ShowCaseList } from 'pages/faculties';
...
function DashboardPage() {
  ...
  const facultyStat = useSWR<PaginatedInterface<FacultyInterface>>(
    () => `/faculties`,
    () => getFaculties({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Faculties',
      result: facultyStat?.data?.totalCount,
      isLoading: facultyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Courses',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`faculties`} passHref legacyBehavior>
                  <Button
